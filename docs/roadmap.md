...
**Quick start на Codeberg.** [README.md](../README.md) Quick start тянет
`docker-compose.yml` и `.env.example` с `raw.githubusercontent.com` (зеркало).
Основной репозиторий — Codeberg; заменить URL на актуальные raw-ссылки
Codeberg (`codeberg.org/mix/selfpost/raw/branch/main/deploy/...`).

**Тег образа в compose.** В [deploy/docker-compose.yml](../deploy/docker-compose.yml)
поле `image:` бампить до версии релиза **в том же коммите**, что и git-тег
`vX.Y.Z` — не раньше. Сейчас может отставать от целевой версии релиза;
несовпадение мешает только до первого выката по тегу.
...
совпадает с релизом; `implementation-plan.md` в `docs/archive/`, ссылок на него
в активных документах и в коде/CI не осталось.
