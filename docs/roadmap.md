...
Жёсткого порядка реализации нет. После `/clear` — пункт со статусом
`согласовано` или `в работе`, затем чеклист в linked plan.
...
каждом хендлере, setup/бэкап. **Версия:** `1.x` MINOR при совместимой миграции
текущего админа в глобального.
...
**Зависимости / риски:** экспорт пакетно-приватного API. **Версия:** `1.x`, сам
по себе breaking не тянет.
